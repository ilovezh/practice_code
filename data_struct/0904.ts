...
// 十进制转为二进制代码
function decimalismTobinary (num:number):string{
    let stack = new Stack();
    while(num != 0){
...
class LinkedList{
    ...
    // 插入节点
    insert(oldNode:EleNode,newNode:EleNode):EleNode{
        let head = this.head;
    ...
    // 查找节点
    searchNode(ele:any):EleNode| undefined{
        let head = this.head;
    ...
    // 查找已知节点的上一个节点
    prev(node:EleNode):EleNode | undefined{
      let head = this.head;
    ...
    // 查找已知节点的下一个节点
    next(node:EleNode):EleNode | undefined{
    ...
    // 删除节点
    deleteNode(node:EleNode):EleNode | void{
        let element:EleNode | undefined = this.searchNode(node.ele);
        // 删除节点不存在
        if(!element) throw new Error("该节点不存在")
        // 如果删除节点为头节点
         ...
         if(element && element.next == undefined){
            // 删除尾节点
            let prev:EleNode = this.prev(element) as EleNode
            prev.next = undefined;
        }
         // 删除一般节点，非头尾节点
...
class Queue{
    ...
    enter(element:any):number{
     this.data.push(element);
     return this.data.length;
    }
    leave():any{
     let element:any = this.data.shift();
     return element;
    }
    search(element:any):number{
